...
Run `prime-agent mcp-serve` to expose the local agent fleet to a trusted MCP client; its HTTP transport has no authentication. See [docs/mcp-integrations.md](docs/mcp-integrations.md) for generic integrations, wrapper APIs, remote-control tools, and security guidance.
