...
import { REPL_PROTOCOL_MAX_PENDING_BYTES } from "../src/core/kernel/repl-manager.js";
import {
...
input.on("line", (line) => {
  ...
  if (request.type === "execute") {
    ...
    if (request.code === "corrupt-overflow") {
      process.stdout.write("x".repeat(${REPL_PROTOCOL_MAX_PENDING_BYTES + 1}));
      return;
    }
...
describe("ReplKernelManager corrupt protocol repair", () => {
	beforeEach(() => {
		...
		const authority = initializeOrphanProcessJournal(join(tempDir, "orphans.jsonl"));
	...
	it("bounds a delimiter-free protocol line and repairs without replaying the cell", async () => {
		const { manager, countPath } = newManager();
		try {
			const corrupt = manager.execute("corrupt-overflow");
			await expect(corrupt).rejects.toThrow(/protocol line exceeded its raw byte bound/);
			await expect(corrupt).rejects.toThrow(/may or may not have run; it was not replayed/);

			const result = await manager.execute("read");
			expect(result).toMatchObject({ status: "ok", stdout: "fresh" });
			expect(spawnCount(countPath)).toBe(2);
		} finally {
			await manager.shutdown({ snapshot: true, drainHostRequests: true });
		}
	...
	it("repairs a partial request write without replaying the cell", async () => {
		const { manager, countPath } = newManager();
		try {
			await manager.start();
			const child = Reflect.get(manager, "child") as ChildProcess | undefined;
			const stdin = child?.stdin;
			if (!stdin) throw new Error("fake kernel stdin is unavailable");
			const originalWrite = stdin.write.bind(stdin);
			stdin.write = ((chunk: string | Uint8Array, callback?: (error?: Error | null) => void): boolean => {
				const text = typeof chunk === "string" ? chunk : Buffer.from(chunk).toString("utf8");
				originalWrite(text.slice(0, Math.max(1, Math.floor(text.length / 2))));
				callback?.(Object.assign(new Error("simulated partial write"), { code: "EPIPE" }));
				return false;
			}) as typeof stdin.write;

			await expect(manager.execute("seed")).rejects.toThrow(/may or may not have run; it was not replayed/);

			// The replacement starts from a fresh namespace. Replaying "seed" would
			// change this to "persisted" and/or increase the spawn count again.
			const result = await manager.execute("read");
			expect(result).toMatchObject({ status: "ok", stdout: "fresh" });
			expect(spawnCount(countPath)).toBe(2);
		} finally {
