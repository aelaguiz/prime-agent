...
describeIf("forkserver kill/liveness protocol (stub python)", () => {
	...
	it("atomically records a forked child's exit status at the requested path", async () => {
		const exitPath = join(tempDir, "kernel-exit.json");
		const handle = await server!.spawnKernel({
			connectionPath: join(tempDir, "exit-status-conn.json"),
			exitPath,
		});
		leakedPids.push(handle.pid);
		expect(await handle.kill("TERM")).toBe("signaled");
		...
		const status = JSON.parse(readFileSync(exitPath, "utf8")) as {
			pid: number;
			code: number | null;
			signal: number | null;
		};
		expect(status).toMatchObject({ pid: handle.pid, code: null });
		expect(status.signal).toBeTypeOf("number");
		expect(status.signal).toBeGreaterThan(0);
		expect(existsSync(`${exitPath}.${handle.pid}.tmp`)).toBe(false);
	}, 15_000);
