...
export class DaemonCatalogClient {
	...
	private async spawnCatalog(): Promise<void> {
		let command: string;
		let args: string[];
		let catalogEnvironment = createCliSubprocessEnv({ ...process.env, [DAEMON_CATALOG_ROLE_ENV]: "1" });
		if (isBunBinary) {
			...
			command = launch.command;
			args = launch.args;
			catalogEnvironment = createCliSubprocessEnv(catalogEnvironment, catalogEntry, execArgs);
		}
