...
const REPL_CONTROL_PROMPT = [
	...
	"",
	"`bash(command)` starts a shell command in the background and returns a handle immediately: `h = bash('npm test')`. Use `h.pid` / `h.running` for liveness, `h.tail(n)` / `h.output()` for combined stdout+stderr so far, `h.poll()` for a non-blocking result, `h.kill()` to terminate (SIGTERM, escalating to SIGKILL). On Windows, the shell is created suspended and atomically inside a non-breakaway kill-on-close Job before user code can run; `bash()` refuses execution if that containment cannot be established. Job termination is primary, and bounded `taskkill /T` is used only after Job termination failure and never counts as Job-empty proof. Use `await h` (or `await bash('cmd')`) for the completed result with exit_code, output, and duration. Prefer bash() for long-running commands so the turn keeps working. Run shell commands with `bash()`, not `subprocess`/`os.system`: subprocess calls block the kernel, show the user nothing while they run, and spawn processes the harness cannot see or stop.",
	"",
