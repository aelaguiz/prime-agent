...
export class AgentSession {
	...
	async deleteInactiveRlmSubagent(
		...
		const result = await this._trackRlmSubagentDeletion(subagent, (isHostFamilyBusy) => {
			if (isRunning() || isHostFamilyBusy()) {
	...
	private async _trackRlmSubagentDeletion(
		subagent: RlmSubagentRegistryEntry,
		startDeletion: (isHostFamilyBusy: () => boolean) => Promise<RlmDeleteSubagentResult>,
		...
		const beginDeletion = () => {
			...
			return runtimeHost?.withRlmSubagentDeletion
				? runtimeHost.withRlmSubagentDeletion(subagent.rlm_child_id, startDeletion)
				: startDeletion(() => false);
		};
		const deletion = Promise.resolve().then(beginDeletion);
	...
	private _ensureRlmRunDeletionCleanup(run: RlmChildRun, session: AgentSession): Promise<void> {
		...
		try {
			// Invoke immediately so a host fence is acquired before the outer
			// deletion admission can return, while preserving rejection semantics
			// for test/custom hosts that throw before returning a promise.
			cleanup = this._deleteRlmSubagentSession(run.id, session);
		} catch (error) {
