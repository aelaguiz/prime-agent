...
event. The TypeScript host and Python runtime must deploy atomically at this
version; mixed versions fail closed and have no compatibility identity path.
Exact admission is supported on Linux, macOS, and Windows. Other platforms,
including BSDs, fail before launch because this protocol has no exact observer there.
Linux identity is the lowercase boot UUID plus canonical unsigned `/proc/<pid>/stat`
start ticks. It is contract-exact only on the same host and PID namespace, at that
kernel's USER_HZ granularity; it is not a global nonce or held process handle.
