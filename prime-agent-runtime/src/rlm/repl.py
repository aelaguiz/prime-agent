...
from .bash import _configured_kernel_lineage, _kill_live_handles
...
def _validate_configured_kernel_admission() -> None:
    admission_keys = (
    ...
    if any(os.environ.get(key) is not None for key in admission_keys) and _configured_kernel_lineage() is None:
        raise RuntimeError("kernel admission protocol/version or exact identity mismatch")


def main() -> None:
    global _loop, _serve_task
    _validate_configured_kernel_admission()
    stdin_fd = _setup_fds()
