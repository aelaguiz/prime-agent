...
class ReplProcess:
    ...
    def __init__(self, env: dict[str, str] | None = None) -> None:
        ...
        for key in (
            ...
            "PRIME_AGENT_INTERNAL_KERNEL_ADMISSION_PROTOCOL",
            "PRIME_AGENT_INTERNAL_KERNEL_ADMISSION_GENERATION",
            "PRIME_AGENT_INTERNAL_KERNEL_LINEAGE",
            "PRIME_AGENT_INTERNAL_KERNEL_PID",
            "PRIME_AGENT_INTERNAL_KERNEL_PROCESS_START_ID",
...
class ReplTest(unittest.TestCase):
    ...
    def test_mixed_version_admission_fails_before_ready(self):
        import rlm.repl as repl_module

        old_ts_env = {
            "PRIME_AGENT_INTERNAL_ORPHAN_PROCESS_JOURNAL": "/old-ts/journal",
            "PRIME_AGENT_INTERNAL_ORPHAN_PROCESS_JOURNAL_GENERATION": "old-generation",
            "PRIME_AGENT_INTERNAL_KERNEL_ADMISSION_PROTOCOL": "1",
            "PRIME_AGENT_INTERNAL_KERNEL_ADMISSION_GENERATION": "12345678-1234-4234-8234-123456789abc",
            "PRIME_AGENT_INTERNAL_KERNEL_LINEAGE": "a" * 64,
            "PRIME_AGENT_INTERNAL_KERNEL_PID": str(os.getpid()),
            "PRIME_AGENT_INTERNAL_KERNEL_PROCESS_START_ID": "proc:10",
        }
        with mock.patch.dict(os.environ, old_ts_env, clear=False):
            with self.assertRaisesRegex(RuntimeError, "protocol/version or exact identity mismatch"):
                repl_module._validate_configured_kernel_admission()
    ...
    def test_input_sees_eof_without_consuming_the_next_protocol_cell(self):
        first = json.dumps(
            {
                "type": "execute",
                "id": "input-eof",
        ...
        second = json.dumps({"type": "execute", "id": "after-input", "code": "6 * 7"})
        assert self.repl.proc.stdin is not None
        self.repl.proc.stdin.write(first + "\n" + second + "\n")
        self.repl.proc.stdin.flush()

        input_events = self.repl.until_done("input-eof")
        self.assertEqual(one(input_events, "result")["text"], "'EOF'")
        self.assertEqual(one(input_events, "done")["status"], "ok")
        next_events = self.repl.until_done("after-input")
        self.assertEqual(one(next_events, "result")["text"], "42")
        self.assertEqual(one(next_events, "done")["status"], "ok")
